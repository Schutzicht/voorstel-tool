    ...
    <div className="space-y-3">
      ...
      {items.map(item => (
        <div key={item.id} className="grid grid-cols-[1fr,auto,auto,auto] gap-2 items-center">
          ...
          <input type="text" value={item.typicalPrice} onChange={e => updItem(item.id, 'typicalPrice', e.target.value)} placeholder="€0" className="w-24 text-sm bg-white border border-warm-grey rounded-xl px-2 py-2.5 outline-none focus:border-indigo text-center" />
          <input type="text" value={item.agenseaPrice} onChange={e => updItem(item.id, 'agenseaPrice', e.target.value)} placeholder="€0" className="w-24 text-sm bg-white border border-indigo/30 rounded-xl px-2 py-2.5 outline-none focus:border-indigo text-center text-indigo font-bold" />
